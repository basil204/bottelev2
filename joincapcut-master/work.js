...
// Function để load cookie từ file
async function loadCookieFromFile(email) {
  try {
    const cookieFile = `cookies_${email.replace('@', '_').replace('.', '_')}.txt`;
    if (fs.existsSync(cookieFile)) {
      const cookie = fs.readFileSync(cookieFile, 'utf8');
      console.log(`📁 Đã load cookie từ file: ${cookieFile}`);
      return cookie;
    } else {
      console.error(`❌ File cookie không tồn tại: ${cookieFile}`);
      return null;
    }
  } catch (error) {
    console.error(`❌ Lỗi đọc file cookie:`, error.message);
    return null;
  }
}

// Function để chạy work.js với thông tin đầy đủ
async function runWork(email, inviteLink, proxyUrl = null) {
  console.log(`\n=== Chạy work.js cho ${email} ===`);
  console.log(`Invite Link: ${inviteLink}`);
  console.log(`Proxy: ${proxyUrl || 'Không'}`);
  
  // Load cookie từ file
  const cookieHeader = await loadCookieFromFile(email);
  if (!cookieHeader) {
    console.error('❌ Không thể load cookie. Vui lòng đăng nhập trước.');
    return;
  }
  
  // Gọi joinWorkspace
  try {
    const result = await joinWorkspace(cookieHeader, inviteLink, proxyUrl);
    console.log('✅ Work.js hoàn thành thành công!');
    return result;
  } catch (error) {
    console.error('❌ Work.js thất bại:', error.message);
    throw error;
  }
}

// Export functions
export { joinWorkspace, loadCookieFromFile, runWork };
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (args.length < 2) {
    ...
    console.log('node work.js <email> <inviteLink> [proxyUrl]');
    console.log('');
    console.log('Ví dụ:');
    console.log('node work.js user@example.com "https://www.capcut.com/..." "proxy:port"');
    console.log('');
    console.log('Hoặc import và sử dụng:');
    console.log('import { runWork } from "./work.js"');
    console.log('await runWork(email, inviteLink, proxyUrl)');
    process.exit(1);
  }
  
  const [email, inviteLink, proxyUrl] = args;
  
  // Chạy work.js
  runWork(email, inviteLink, proxyUrl)
    .then(() => {
