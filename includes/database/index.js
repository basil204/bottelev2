...
export const initDb = async (config) => {
  ...
  try {
    ...
    // Ensure canva_teams & canva_tasks tables exist
    try {
      await pool.execute(`
      ...
      `);
      await pool.execute("INSERT IGNORE INTO settings (`key`, `value`) VALUES ('canva_enabled', 'true')");
      await pool.execute("INSERT IGNORE INTO settings (`key`, `value`) VALUES ('canva_price', '15000')");
      await pool.execute("INSERT IGNORE INTO settings (`key`, `value`) VALUES ('canva_headless', 'true')");
      await pool.execute("INSERT IGNORE INTO settings (`key`, `value`) VALUES ('canva_concurrency', '1')");
      await pool.execute("INSERT IGNORE INTO settings (`key`, `value`) VALUES ('canva_default_role', 'member')");
      await pool.execute("INSERT IGNORE INTO settings (`key`, `value`) VALUES ('canva_proxies', '')");
      console.log('✅ Bảng canva_teams và canva_tasks sẵn sàng');
    } catch (e) {
      console.error('Migration error for canva tables:', e.message);
    }
